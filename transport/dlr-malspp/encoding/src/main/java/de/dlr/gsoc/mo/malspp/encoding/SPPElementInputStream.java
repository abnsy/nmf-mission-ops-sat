...
public class SPPElementInputStream implements MALElementInputStream {
  ...
  private Object readElementPubSubPublishUpdate(final Object element, final ServiceInfo service)
      throws MALException {
    ...
    // EncodedUpdateSize <- MALEncodedElement
    //
    // |-----------------------------------^
    //                                                               |<-------------------------
    // BlobList ------------------------>|
    final Long updateListShortForm =
        service.isDeclaredAbstract()
    ...
    final Long updateValueShortForm =
        (updateListShortForm & ~0xFFFFFF) | (-(updateListShortForm & 0xFFFFFF) & 0xFFFFFF);
    final MALEncodedElementList encElemList =
        new MALEncodedElementList(updateValueShortForm, 10); // initial list size set arbitrarily
  ...
  private Object readElementStandard(final Object element, final ServiceInfo service)
      throws MALException {
  ...
  private Object readNullableElementStandard(final Object element, final ServiceInfo service)
      throws MALException {
    if (decoder.isNull()) {
      return null;
    }
