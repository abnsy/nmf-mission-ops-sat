...
public class SPPDecoder implements MALDecoder {
  ...
  protected static final String LENGTH_NOT_SUPPORTED =
      "Field or list length exceeds supported length.";
  ...
  public Duration decodeDuration() throws MALException {
    ...
    final AbsoluteDate epoch =
        CCSDSTime.createEpoch(Configuration.DURATION_EPOCH, Configuration.DURATION_EPOCH_TIMESCALE);
  ...
  public FineTime decodeFineTime() throws MALException {
    ...
    final AbsoluteDate time = tf.getDecodedTime(tField);
    final AbsoluteDate epoch =
        CCSDSTime.createEpoch(
            Configuration.MAL_FINE_TIME_EPOCH, Configuration.MAL_FINE_TIME_EPOCH_TIMESCALE);
    ...
    final long picoSeconds =
        coarseSeconds * 1000000000000L + Math.round(time.durationFrom(coarseTime) * 1000000000000L);
  ...
  public Time decodeTime() throws MALException {
    ...
    // Construct our own Java epoch due to bug in Orekit library
    // (https://www.orekit.org/forge/issues/142).
    final AbsoluteDate epoch =
        CCSDSTime.createEpoch(Configuration.JAVA_EPOCH, Configuration.JAVA_EPOCH_TIMESCALE);
  ...
  @Override
  public Element decodeElement(final Element element)
  ...
  @Override
  public Element decodeNullableElement(final Element element)
      throws IllegalArgumentException, MALException {
  ...
  @Override
  public MALListDecoder createListDecoder(final List list)
      throws IllegalArgumentException, MALException {
    if (list == null) {
      throw new IllegalArgumentException(ILLEGAL_NULL_ARGUMENT);
    }
   ...
   * (false).
   *
   * @return True if presence flag indicates null value (i.e. if the flag evaluates to false), false
   *     otherwise.
   * @throws MALException
   */
  ...
  /**
   * Returns a number n bytes from inputStream. If less than n bytes are available a MALExcpetion is
   * thrown.
   *
  ...
  /**
   * Decodes a variable integer from inputStream. Respects the parameter VARINT_SUPPORTED and reads
   * in a non-variable integer in case Varints are not supported.
   *
   * @param nOctets Number of bytes of the encoded value. Note that this is not the number of bytes
   *     that need to be read from the stream in case of variable integers.
  ...
  private CCSDSTime getTimeFormatter() throws MALException {
    if (timeFormatter == null) {
      ...
      timeFormatter =
          new CCSDSTime(
              config.timeCodeFormat(),
              config.timeEpoch(),
              config.timeEpochTimescale(),
              config.timeUnit());
    }
  ...
  private CCSDSTime getFineTimeFormatter() throws MALException {
    if (fineTimeFormatter == null) {
      ...
      fineTimeFormatter =
          new CCSDSTime(
              config.fineTimeCodeFormat(),
              config.fineTimeEpoch(),
              config.fineTimeEpochTimescale(),
              config.fineTimeUnit());
    }
   ...
   *
   * <p>This method uses an arbitrary epoch (DURATION_EPOCH) to map relative times to absolute times
  ...
  private CCSDSTime getDurationFormatter() throws MALException {
    if (durationFormatter == null) {
      ...
      durationFormatter =
          new CCSDSTime(
              config.durationCodeFormat(),
              Configuration.DURATION_EPOCH,
