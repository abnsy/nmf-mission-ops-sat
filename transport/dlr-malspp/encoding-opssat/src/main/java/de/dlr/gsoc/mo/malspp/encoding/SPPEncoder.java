...
public class SPPEncoder implements MALEncoder, MALListEncoder {
  ...
  @Override
  public MALListEncoder createListEncoder(final List list)
   ...
   *
   * @param obj A presence flag with value false is encoded, if obj is null, otherwise the presence
   *     flag is encoded with value true.
   * @throws MALException
   */
   ...
   * @param signed True, if the values are signed and in case of variable integers shall be zig-zag
   *     mapped before encoding. False if no mapping shall be employed. Ignored for non- variable
   *     integers.
   * @throws IOException
   */
  private void writeVarint(BigInteger value, final int nOctets, final boolean signed)
      throws MALException {
    if (!varintSupported) {
    ...
    while (true) {
      ...
      write(
          value.and(BigInteger.valueOf(0b01111111)).or(BigInteger.valueOf(0b10000000)).intValue());
