...
public class SPPElementOutputStream implements MALElementOutputStream {
  ...
  protected static final String INVALID_ELEMENT_TYPE =
      "Supplied element type cannot be handled by the transport layer.";
  ...
  public void writeElement(final Object element, final MALEncodingContext ctx)
    ...
    } else if ((isPubSub && !service.isErrorMessage())
  ...
  private void writeElementPubSubPublishUpdate(final Object element, final ServiceInfo service)
      throws MALException {
    // Only MAL element types need to be handled here, because this case does not happen
    // in a broker.
    // Body Element [PubSub/Publish/ListOfUpdates] -> (Type info) -> ListLength -> NullTag ->
    // EncodedUpdateSize -> Element
    //
    // ^-------------------------------|
    switch (getElementType(element)) {
      case NULL:
  ...
  private void writeElementPubSubNotifyUpdate(final Object element, final ServiceInfo service)
      throws MALException {
    ...
    // Body Element [PubSub/Notify/ListOfUpdates] -> (Type info) -> ListLength -> NullTag ->
    // MALEncodedElement
    //
    // ^--------------|
    ...
    switch (getElementType(element)) {
      ...
      case ENCODED_LIST:
        ...
        // flipped type.
        final long updateListShortForm =
            (updateValueShortForm & ~0xFFFFFF) | (-(updateValueShortForm & 0xFFFFFF) & 0xFFFFFF);
        if (service.isDeclaredAbstract()) {
  ...
  private void writeNullableElementStandard(Object element, final ServiceInfo service)
      throws MALException {
    // Only element types need to be handled here. They may be abstract.
   ...
   *     mapping type, ENCODED_LIST if the element is a MALEncodedList, ENCODED_ELEMENT if the
   *     element is an MALEncodedElement.
   * @throws MALException
   */
   ...
   *
   * @param element Element that is one of the allowed Java mapping types. A MALException is thrown,
