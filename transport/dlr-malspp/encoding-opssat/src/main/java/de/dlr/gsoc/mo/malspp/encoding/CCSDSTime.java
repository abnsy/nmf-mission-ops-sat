...
public class CCSDSTime {
  ...
  static {
    try {
      org.orekit.data.DataProvidersManager.getInstance()
          .addProvider(new org.orekit.data.ClasspathCrawler(OREKIT_UTC_TAI_FILE));
   ...
   *
   * @param pField Preamble field as byte array. A one or two-byte length array has to be provided.
   ...
   * @param timeScale Timescale in which the epoch is interpreted if a CUC encoding is employed. See
   *     getTimeScale for possible values. UTC if null.
   * @param unitMultiplier Number of time units that make up a second. Only used for CUC encoding.
   * @throws MALException
   */
  public CCSDSTime(
      final byte[] pField, final String epoch, final String timeScale, final int unitMultiplier)
      throws MALException {
  ...
  /**
   * Create a new CCSDS Time Code object by parsing a single- or multioctet preamble field given in
   ...
   */
  public CCSDSTime(
      final String pField, final String epoch, final String timeScale, final int unitMultiplier)
      throws MALException {
   ...
   *
   * @param epoch ISO-8601 formatted epoch string (only the subset allowed by Orekit is supported).
   ...
   * @throws MALException
   */
  public static AbsoluteDate createEpoch(final String epoch, final String timeScale)
      throws MALException {
   ...
   *
   * <p>If the epoch is an agency-defined epoch the epoch data member is set to null.
   *
   ...
   *
   * <p>If there is a mismatch between parsed epoch and the epoch parameter a MALException is
   * thrown.
   *
   * @param epoch Epoch as ISO-8601 formatted String as supported by the Orekit library. The CCSDS
   *     ASCII calendar segmented time code is a supported subset of ISO-8601.
    ...
    try {
      switch (timeCode) {
        case CUC:
          ...
          final AbsoluteDate possiblyWrongTime =
              AbsoluteDate.parseCCSDSUnsegmentedTimeCode(
                  pField[0], pField.length == 2 ? pField[1] : 0, tField, epoch);
          if (1 == unitMultiplier) {
        ...
        case CDS:
          final DateComponents dEpoch =
              epoch
                  .getComponents(TimeScalesFactory.getUTC())
                  .getDate(); // CDS is a UTC-based timecode
   ...
   *
   * <p>The basic unit for CUC is assumed to be one second. Beware that while a resolution of up to
  ...
  public byte[] getEncodedTime(
      final AbsoluteDate coarseTime, final AbsoluteDate fineTime, final boolean mayBeNegative)
      throws MALException {
   ...
   * @throws MALException
   */
  private byte[] encodeCUC(final AbsoluteDate coarseTime, final AbsoluteDate fineTime)
      throws MALException {
    ...
    final long fineSeconds =
        Math.round(
            fineTime.durationFrom(coarseTime) * unitMultiplier * (1L << (8 * nFractionalOctets)));
  ...
  private byte[] encodeCDS(final AbsoluteDate time) throws MALException {
    ...
    final int days =
        dateTime.getDate().getJ2000Day() - epoch.getComponents(utc).getDate().getJ2000Day();
