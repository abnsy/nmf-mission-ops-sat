...
public class SPPElementInputStreamTest extends SPPElementIOStreamTestHelper {
  ...
  public static void setUpClass() {
    MALElementFactory elementFactoryInteger =
        new MALElementFactory() {
          @Override
          public Object createElement() {
            return new Integer(0);
          }
        };
    MALElementFactory elementFactoryUOctet =
        new MALElementFactory() {
          @Override
          public Object createElement() {
            return new UOctet();
    ...
    MALElementFactory elementFactoryIdBooleanPair =
        new MALElementFactory() {
          @Override
          public Object createElement() {
    ...
    MALElementFactoryRegistry elementFactoryRegistry =
        MALContextFactory.getElementFactoryRegistry();
    elementFactoryRegistry.registerElementFactory(
        Attribute.INTEGER_SHORT_FORM, elementFactoryInteger);
    elementFactoryRegistry.registerElementFactory(
        Attribute.UOCTET_SHORT_FORM, elementFactoryUOctet);
    elementFactoryRegistry.registerElementFactory(
        IdBooleanPair.SHORT_FORM, elementFactoryIdBooleanPair);
  }
  ...
  protected void performTest(
      ...
      byte[] buffer)
      throws Exception {
    // System.out.println("IS testing: " + testContext.toString() + ", actual " +
    // actualType.toString() + ", declared " + declaredType.toString());
    ...
    if (actualType == TestActualType.MAL_ENCODED_ELEMENT_LIST) {
      ...
      assertEquals(
          ((MALEncodedElementList) d.element).size(), ((MALEncodedElementList) element).size());
  ...
  public void testReadElementPubSub1() throws Exception {
    try {
      performTest(
          TestContext.PUBSUB, TestActualType.NULL, TestDeclaredType.CONCRETE, new byte[] {0});
  ...
  public void testReadElementPubSubPublish() throws Exception {
    performTest(
        TestContext.PUBSUB_PUBLISH_UPDATE,
        TestActualType.MAL_ENCODED_ELEMENT_LIST,
        TestDeclaredType.CONCRETE,
        new byte[] {4, 0, 1, 1, 11, 1, 1, 13, 1, 1, 17});
  }
}
