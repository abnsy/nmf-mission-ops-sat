...
public class SPPMessageHeader implements MALMessageHeader {
  ...
  private static final String VERSION_NOT_SUPPORTED =
      "Received Space Packet with unsupported MAL/SPP version number.";
  private URI uriFrom;
  ...
  private final Integer
      offset; // only used when constructing header from Space Packet; denotes body data
  // offset
  ...
  public SPPMessageHeader(
      final SpacePacket spacePacket, final MALElementStreamFactory esf, final Map msgQosProperties)
      throws MALException {
  ...
  private int initMessageHeader(
      final ByteArrayInputStream is,
      final boolean isTCpacket,
      final int sequenceFlag,
      final short primaryApid,
      final int primaryApidQualifier,
      final MALElementStreamFactory esf,
      final Map msgQosProperties)
      throws MALException {
    ...
    final Short sourceIdentifier =
        bs.get(7) ? ((UOctet) eis.readElement(new UOctet(), null)).getValue() : null;

    final Short destinationIdentifier =
        bs.get(6) ? ((UOctet) eis.readElement(new UOctet(), null)).getValue() : null;
    ...
    final UInteger priority =
        bs.get(5) ? (UInteger) eis.readElement(new UInteger(), null) : config.priority();
    setPriority(priority);

    final Time timestamp =
        bs.get(4) ? (Time) eis.readElement(new Time(), null) : Configuration.DEFAULT_TIMESTAMP;
    setTimestamp(timestamp);

    final Identifier networkZone =
        bs.get(3) ? (Identifier) eis.readElement(new Identifier(), null) : config.networkZone();
    setNetworkZone(networkZone);

    final Identifier sessionName =
        bs.get(2) ? (Identifier) eis.readElement(new Identifier(), null) : config.sessionName();
    setSessionName(sessionName);

    final IdentifierList domain =
        bs.get(1) ? (IdentifierList) eis.readElement(new IdentifierList(), null) : config.domain();
    setDomain(domain);

    final Blob authenticationId =
        bs.get(0) ? (Blob) eis.readElement(new Blob(), null) : config.authenticationId();
  ...
  /**
   * Gets the SDU type of the Space Packet according to Table 3-6 of the MAL/SPP Book.
   *
  ...
  private static final UOctet[] SDU_STAGES =
      new UOctet[] {
        new UOctet((short) 0), // interaction stage for SEND not
        // specified, set to 0
  ...
  /**
   * Helper method for reading an unsigned short value from a MALElementInputStream using a fixed
  ...
  /**
   * Helper method for reading a signed long value from a MALElementInputStream using a fixed
