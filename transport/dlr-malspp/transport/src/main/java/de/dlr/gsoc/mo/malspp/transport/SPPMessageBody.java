  ...
  public Object getBodyElement(final int index, final Object element) throws MALException {
    if (!isDecoded) {
      ...
      if (getElementCount() != 0) {
        ...
        final MALElementFactoryRegistry elementFactoryRegistry =
            MALContextFactory.getElementFactoryRegistry();
        final MALElementInputStream is =
            esf.createInputStream(
                encodedBody.getEncodedBody().getValue(), encodedBody.getEncodedBody().getOffset());
        for (int i = 0; i < shortForms.length; i++) {
          ...
          try {
            ...
            //
            // Logger.getLogger(SPPMessageBody.class.getName()).log(Level.INFO, "Unable to decode
            // element with index: " + i, ex);
