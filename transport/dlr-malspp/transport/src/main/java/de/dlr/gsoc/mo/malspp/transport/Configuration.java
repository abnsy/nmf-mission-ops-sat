...
public class Configuration {
  ...
  private static final String PROPERTY_AUTHENTICATION_ID =
      "de.dlr.gsoc.mo.malspp.AUTHENTICATION_ID";
  ...
  private static final String PROPERTY_PACKET_DATA_FIELD_SIZE_LIMIT =
      "de.dlr.gsoc.mo.malspp.PACKET_DATA_FIELD_SIZE_LIMIT";
  ...
  private static final String PROPERTY_MAPPING_CONFIGURATION_FILE =
      "de.dlr.gsoc.mo.malspp.MAPPING_CONFIGURATION_FILE";
  ...
  private static final String PROPERTY_AUTHENTICATION_ID_FLAG =
  ...
  private static final String PROPERTY_NETWORK_ZONE_FLAG =
      "org.ccsds.moims.mo.malspp.networkZoneFlag";
  private static final String PROPERTY_PRIORITY_FLAG = "org.ccsds.moims.mo.malspp.priorityFlag";
  private static final String PROPERTY_SESSION_NAME_FLAG =
      "org.ccsds.moims.mo.malspp.sessionNameFlag";
  ...
  private static final String PROPERTY_TIME_EPOCH_TIMESCALE =
      "de.dlr.gsoc.mo.malspp.TIME_EPOCH_TIMESCALE";
  private static final String PROPERTY_TIME_UNIT = "de.dlr.gsoc.mo.malspp.TIME_UNIT";
  private static final String PROPERTY_FINE_TIME_CODE_FORMAT =
      "de.dlr.gsoc.mo.malspp.FINE_TIME_CODE_FORMAT";
  private static final String PROPERTY_FINE_TIME_EPOCH = "de.dlr.gsoc.mo.malspp.FINE_TIME_EPOCH";
  private static final String PROPERTY_FINE_TIME_EPOCH_TIMESCALE =
      "de.dlr.gsoc.mo.malspp.FINE_TIME_EPOCH_TIMESCALE";
  private static final String PROPERTY_FINE_TIME_UNIT = "de.dlr.gsoc.mo.malspp.FINE_TIME_UNIT";
  private static final String PROPERTY_DURATION_CODE_FORMAT =
      "de.dlr.gsoc.mo.malspp.DURATION_CODE_FORMAT";
  ...
  public Blob authenticationId() {
    ...
    return properties.get(PROPERTY_AUTHENTICATION_ID) == null
  ...
  /**
   * Get a property flag from its name and return its boolean value. Accepts Boolean objects and
   * Strings as value in the property map.
   *
   * @param flagProperty
   * @return
   */
  private boolean getBooleanProperty(final String flagProperty) {
    ...
    return (null == propertyObject)
        || (propertyObject instanceof Boolean
            ? (Boolean) propertyObject
            : Boolean.valueOf((String) propertyObject));
  }

  /**
   * Return a map containing all properties that are effective for a specific application identified
   * by APID qualifier and APID. Please note that the application is not necessarily identified by
   * the sending or receiving endpoint but is dependent on the packet type (TM/TC).
   *
   * @param primaryQualifier The APID qualifier to use for retrieving the effective configuration.
   * @param primaryApid The APID to use for retrieving the effective configuration.
   * @return A map containing the effective configuration.
   */
  protected synchronized Map getEffectiveProperties(
      final int primaryQualifier, final short primaryApid) throws MALException {
          ...
          if (Scope.START == s && Scope.CONFIG.getName().equals(name)) {
            ...
            try {
              final int qualifier =
                  Integer.parseInt(
                      e.asStartElement()
                          .getAttributeByName(QName.valueOf(XML_ATTR_APID_QUALIFIER))
                          .getValue());
              final short apid =
                  Short.parseShort(
                      e.asStartElement()
                          .getAttributeByName(QName.valueOf(XML_ATTR_APID))
                          .getValue());
            ...
            if (Scope.FORMAT.getName().equals(name)) {
              final String v =
                  e.asStartElement()
                      .getAttributeByName(QName.valueOf(XML_ATTR_TIME_UNIT))
                      .getValue();
              ...
              final String v =
                  e.asStartElement()
                      .getAttributeByName(QName.valueOf(XML_ATTR_TIME_SCALE))
                      .getValue();
  ...
  /**
   * Convert the hexadecimal digits in the String parameter to a byte array. Two hex digits
   * represent one byte, therefore the number of digits has to be even.
   *
  ...
  /**
   * Returns a newly created Map containing the union of two property maps. If a value is present in
   * both maps the value of newProperties will be taken. Any map may be null.
   *
   * @param baseProperties Base property map.
   * @param newProperties New property map. Will overwrite values of baseProperties.
  ...
  private enum Scope {
    ...
    PACKET_DATA_FIELD_SIZE_LIMIT("packetDataFieldSizeLimit"),
    VARINT_SUPPORTED("varintSupported");
