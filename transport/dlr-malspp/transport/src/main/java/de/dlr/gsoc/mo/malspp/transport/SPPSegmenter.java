...
public class SPPSegmenter implements Iterator {
  ...
  private static final String TOO_SMALL =
      "SPACE_PACKET_SIZE_LIMIT too small to accomodate secondary header and at least one octet of"
          + " user data.";
   ...
   *
   * <p>No error checking is performed. If the Space Packet is malformed due to a body that is too
  ...
  private static long getSegmentCounter(final SpacePacket spacePacket) {
    ...
    final int counter_pos =
        FLAG_IDX + 1 + ((flags & 0b10000000) >>> 7) + ((flags & 0b01000000) >>> 6);
  ...
  public static SpacePacket[] split(
    ...
    final Iterator<Long> segmentCounterIter =
        (numberOfPackets > 1) ? segmentCounter.increment(numberOfPackets) : null;
    ...
    do {
      ...
      final SpacePacket spacePacket =
          new SpacePacket(spHeader, primaryApidQualifier, os.toByteArray(), 0, os.size());
