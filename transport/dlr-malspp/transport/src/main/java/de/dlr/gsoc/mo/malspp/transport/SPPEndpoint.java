...
public class SPPEndpoint implements MALEndpoint {
  ...
  private static final String HEADER_FIELD_IS_NULL =
      "At least one non-nullable MAL header field is null.";
  ...
  public MALMessage createMessage(
    ...
    final MALOperation op =
        MALContextFactory.lookupArea(serviceArea, areaVersion)
  ...
  private MALMessage createMessage(
    ...
    final Configuration config =
        new Configuration(Configuration.mix(this.effectiveQosProperties, qosProperties));
    final SPPURI primarySPPURI = new SPPURI(config.isTCpacket() ? uriTo : uriFrom);
    final Map props =
        config.getEffectiveProperties(primarySPPURI.getQualifier(), primarySPPURI.getAPID());
    final MALEncodingContext ctx =
        new MALEncodingContext(msgHeader, op, -1, this.effectiveQosProperties, props);
    final MALElementStreamFactory esf = MALElementStreamFactory.newFactory(protocol, props);
    final SPPMessageBody msgBody =
        isEncoded ? createMessageBody(encBody, esf, ctx) : createMessageBody(body, esf, ctx);
    return new SPPMessage(msgHeader, msgBody, props, this.qosProperties, esf, transport);
  }
  // </editor-fold>

  /**
   * Creates an error message in reply to another MAL message. If returning an error message is not
   * allowed by the interaction type and stage of the original MAL message, then null is returned.
   *
   ...
   *     types.
   * @param uriFrom The URI used for the sender. If null, the URI of this endpoint is used,
   *     otherwise the endpoint URI won't be considered for this error message and is overwritten.
   * @return A MALMessage representing the error, with the isErrorMessage flag set, the timestamp of
   *     the error generation and the correct interaction stage for returning an error. Null, if it
   *     is not allowed to return a message.
   * @throws MALException
   */
  protected MALMessage createErrorMessage(
      final MALMessage replyToMsg, final MALStandardError error, final URI uriFrom)
      throws MALException {
    ...
    final boolean isErrorAllowed =
        ((type == InteractionType._SUBMIT_INDEX && stage == MALSubmitOperation._SUBMIT_STAGE)
            || (type == InteractionType._REQUEST_INDEX
                && stage == MALRequestOperation._REQUEST_STAGE)
            || (type == InteractionType._INVOKE_INDEX && stage == MALInvokeOperation._INVOKE_STAGE)
            || (type == InteractionType._PROGRESS_INDEX
                && stage == MALProgressOperation._PROGRESS_STAGE)
            || (type == InteractionType._PUBSUB_INDEX
                && stage == MALPubSubOperation._REGISTER_STAGE)
            || (type == InteractionType._PUBSUB_INDEX
                && stage == MALPubSubOperation._PUBLISH_REGISTER_STAGE)
            || (type == InteractionType._PUBSUB_INDEX
                && stage == MALPubSubOperation._PUBLISH_DEREGISTER_STAGE)
            || (type == InteractionType._PUBSUB_INDEX
    ...
    final MALMessage errMsg =
        createMessage(
            ...
            Boolean.TRUE, // Yes, this is an error message.
            replyToMsg.getQoSProperties(),
  ...
  protected static SPPMessageBody createMessageBody(
      final MALEncodedBody encodedBody,
  ...
  /** Creates a specialized message body according to some header information. */
  protected static SPPMessageBody createMessageBody(
      final Object[] body, final MALElementStreamFactory esf, final MALEncodingContext ctx) {
    final MALMessageHeader header = ctx.getHeader();
    if (header.getIsErrorMessage()) {
  ...
  public void sendMessage(final MALMessage msg)
    ...
    if (null == header.getURIFrom()
        ...
        || null == header.getInteractionType()
        || null == header.getInteractionStage()
        || null == header.getTransactionId()
        || null == header.getServiceArea()
        || null == header.getService()
        || null == header.getOperation()
        || null == header.getAreaVersion()
        || null == header.getIsErrorMessage()) {
      final MALStandardError error =
          new MALStandardError(MALHelper.INTERNAL_ERROR_NUMBER, HEADER_FIELD_IS_NULL);
    ...
    try {
      ...
      final MALStandardError error =
          new MALStandardError(MALHelper.INTERNAL_ERROR_NUMBER, INVALID_URI);
      throw new MALTransmitErrorException(header, error, msg.getQoSProperties());
    }
    ...
    try {
      if (isLocalDestination) {
        try {
          ...
          Logger.getLogger(SPPEndpoint.class.getName())
              .log(Level.SEVERE, "Maybe the configuration file is not being read!", ex);
          final MALStandardError error =
              new MALStandardError(MALHelper.INTERNAL_ERROR_NUMBER, ex.getMessage());
      ...
      synchronized (sppSocket) {
        final SPPCounter sequenceCounter =
            transport.getSequenceCounter(primaryQualifier, primaryApid);
        ...
        final SpacePacket[] spacePackets =
            ((SPPMessage) msg)
                .createSpacePackets(sequenceCounter, segmentCounter, packetDataFieldSizeLimit);
