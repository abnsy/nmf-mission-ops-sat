...
public class SPPTransport extends SPPBaseTransport<SpacePacket> {
   ...
   */
  public SPPTransport(
      final String protocol, final MALTransportFactory factory, final java.util.Map properties)
      throws MALException {
    super(
        new SPPConfiguration(true, 65530, true, true, true, true, true, true, true, true),
        new SPPURIRepresentationSimple(),
        new SPPSourceSequenceCounterSimple(),
        protocol,
        ":",
        '/',
        '@',
  ...
  public void init() throws MALException {
    ...
    final GENMessagePoller rcvr =
        new GENMessagePoller<>(
            this,
  ...
  public void close() throws MALException {
    ...
    synchronized (this) {
      try {
        ...
        LOGGER.log(
            Level.WARNING, "Space Packet Socket error (ignore if socket closed on purpose).", ex);
        throw new MALException(
            "Space Packet Socket error (ignore if socket closed on purpose).", ex);
      }
    }
  }
  ...
  public GENMessage createMessage(final SpacePacket packet) throws MALException {
    ...
    final int pkt_ident =
        (vers_nb << 13) | (pkt_type << 12) | (sec_head_flag << 11) | (TCPacket_apid);
