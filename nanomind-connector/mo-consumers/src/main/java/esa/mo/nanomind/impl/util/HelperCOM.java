...
import esa.opssat.nanomind.com.COMObject;
import esa.opssat.nanomind.com.COMService;
import esa.opssat.nanomind.com.archive.structures.ExpressionOperator;
...
public class HelperCOM {
  ...
  public static Boolean evaluateExpression(
      Element leftHandSide, final ExpressionOperator operator, Attribute rightHandSide) {
    ...
    if ((leftHandSide instanceof Blob)
        && (rightHandSide instanceof Blob)) { // Are we comparing Blobs?
      if (operator.equals(ExpressionOperator.EQUAL)) {
    ...
    final boolean stringComparison =
        HelperMisc.isStringAttribute(rightHandSide) || HelperMisc.isStringAttribute(rightHandSide);
   ...
   * @return COMService
   */
  public static COMService findCOMService(
      final UShort area, final UOctet areaVersion, final UShort serviceNumber) {
    return (COMService)
        MALContextFactory.lookupArea(area, areaVersion).getServiceByNumber(serviceNumber);
  }
   ...
   */
  public static boolean domainMatchesWildcardDomain(
      final IdentifierList domain, final IdentifierList wilcardDomain) {
  ...
  /**
   * Generates a COM ObjectType object from the area, service, version and object number. Deprecated
   * because this used to be needed when the services didn't provide the static COM Objects
   * ObjectType. This can now be found in the service Helper. For example, for the Parameter
   * service, the parameter definition can be found in:
   ...
   */
  @Deprecated
  public static ObjectType generateCOMObjectType(
      final int area, final int service, final int version, final int number) {
    return new ObjectType(
        new UShort(area), new UShort(service), new UOctet((short) version), new UShort(number));
  }
  ...
  public static Object getNestedObject(final Object in, final String fieldName)
  ...
  /**
   * Creates a subscription object for the Event service. This subscription selects all the COM
   * Events of a certain service.
   *
   * @param identifier A name identifier for the subscription
   * @param objType The object type containing the area, service, and version of the service to be
   *     selected.
   * @return
   */
  public static Subscription generateSubscriptionCOMEvent(
      final String identifier, final ObjectType objType) {
